...
exports.getStudentDefaultSchedule = async (req, res) => {
  try {
    ...
    // 3️⃣ Get ONLY sections where schedule is approved
    const sections = await sql`
      SELECT 
        s.id AS section_id,
        s.day_of_week,
      ...
      JOIN level lv ON c.level_id = lv.id
      JOIN groups g ON s.section_group = g.id
      JOIN schedule sch ON s.schedule_id = sch.id  -- ✅ link section to schedule
      WHERE c.level_id = ${level_id}
